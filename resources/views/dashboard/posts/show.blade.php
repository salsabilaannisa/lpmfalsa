        ...
        <div class="row my-3">
          <div class="col-lg-8">
            ...
            <form action="/dashboard/posts/confirm/{{ $post->slug }}/0" method="post" class="d-inline">
                    @method('post')
                    @csrf
                    <button class="btn btn-danger" onclick="return confirm('Are you sure?')"><span data-feather="x-circle"></span> Reject</button>
            </form>
            <form action="/dashboard/posts/confirm/{{ $post->slug }}/1" method="post" class="d-inline">
                    @method('post')
                    @csrf
                    <button class="btn btn-success" onclick="return confirm('Are you sure?')"><span data-feather="check-circle"></span> Accept</button>
            </form>
            @endif
